...
until curl -s -f "$ES_HOST" > /dev/null; do
...
# 인덱스가 이미 존재하는지 확인
INDEX_EXISTS=$(curl -s -o /dev/null -w "%{http_code}" "$ES_HOST/perfumes")
...
if [ "$INDEX_EXISTS" = "200" ]; then
  ...
  if [ "$RECREATE" = "y" ]; then
    echo "기존 인덱스 삭제 중..."
    curl -X DELETE "$ES_HOST/perfumes"
...
# 인덱스 생성 및 설정
echo "인덱스 생성 및 설정 중..."
curl -X PUT "$ES_HOST/perfumes" -H "Content-Type: application/json" -d @settings.json
echo "인덱스 설정이 완료되었습니다."

# 매핑 설정
echo "매핑 설정 중..."
curl -X PUT "$ES_HOST/perfumes/_mapping" -H "Content-Type: application/json" -d @mapping.json
echo "매핑 설정이 완료되었습니다."
